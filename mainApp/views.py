...
from vshope.settings import RAZORPAY_API_KEY,RAZORPAY_API_SECRET_KEY
import razorpay
...
def home(Request):
    ...
    return render(Request,"index.html",{"data":data,"car":car})
...
def addToCart(Request,id):
    ...
    if(cart is None):
        cart = {p.id:{"pid":p.id,"pic":p.pic1.url,"name":p.name,"color":p.color,"size":p.size,"price":p.finalprice,"qty":1,"total":p.finalprice,"maincategory":p.maincategory.name,"subcategory":p.subcategory.name,"brand":p.brand.name}}
    else:
        if(str(p.id) in cart):
            item = cart[str(p.id)]
            item['qty']=item['qty']+1
            ...
            cart.setdefault(p.id,{"pid":p.id,"pic":p.pic1.url,"name":p.name,"color":p.color,"size":p.size,"price":p.finalprice,"qty":1,"total":p.finalprice,"maincategory":p.maincategory.name,"subcategory":p.subcategory.name,"brand":p.brand.name})
...
def orderPage(Request):
    if(Request.method=="POST"):
        ...
        if(cart is None):
            ...
            subject = 'Your Order has been Placed: Team Vshop'
    ...
    if(Request.method=="POST"):
        ...
        try:
            ...
            if(user.is_superuser):
                ...
                buyer.otp = otp
                buyer.save()
                subject = 'OTP for Password Reset: Team Vshop'
